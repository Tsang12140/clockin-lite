...
import { getOvertimeMultipliers } from '@/lib/tenant';
import OvertimeForm from './OvertimeForm';

export const dynamic = 'force-dynamic';

export default async function OvertimeSettingsPage() {
  const multipliers = await getOvertimeMultipliers();
    ...
    <div className="min-h-screen bg-[#F0F4FA]">
      <div className="mx-auto max-w-2xl md:px-6 md:py-5">
        <div className="flex items-center gap-3 bg-white px-4 pb-4 pt-5 shadow-sm md:rounded-2xl">
          ...
          <div>
            <h1 className="text-[17px] font-semibold text-[#1A3A8F]">薪资规则</h1>
            <div className="mt-0.5 text-[12px] text-gray-400">正常工时、加班工时、休息日倍率</div>
          </div>
        </div>

        <div className="mt-3 px-3 md:px-0">
          <OvertimeForm initialValues={multipliers} />
        </div>
      </div>
    </div>
  );
