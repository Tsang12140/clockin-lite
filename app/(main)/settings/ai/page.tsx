...
export default function AISettingsPage() {
  ...
  // Toggle: turning OFF saves immediately; turning ON just expands the form.
    ...
    if (enabled) {
      ...
      try {
        ...
        const data = await callApi({
          action: 'save',
          enabled: false,
          provider, baseUrl, model, rulesPrompt,
  ...
  const savePreset = async () => {
    ...
    try {
      const data = await callApi({
        action: 'save-preset',
        notes: presetNotes, provider, baseUrl, model, rulesPrompt,
        apiKey: isKeyPlaceholder ? '' : apiKey,
        presetId: isKeyPlaceholder && selectedPresetId ? selectedPresetId : undefined,
      });
    ...
    <div className="min-h-screen bg-[#F0F4FA]">
      <div className="mx-auto max-w-2xl md:px-6 md:py-5">
        <div className="flex items-center bg-white px-4 pb-4 pt-5 shadow-sm md:rounded-2xl">
          ...
          <h1 className="text-[17px] font-semibold text-[#1A3A8F]">AI 助手</h1>
        </div>
        ...
        <div className="space-y-3 px-3 py-3 md:px-0">
          ...
          <div className="rounded-2xl bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[15px] font-semibold text-gray-800">AI 助手</div>
              ...
              <button
                ...
                disabled={busy}
                className={`relative h-7 w-[52px] rounded-full transition-colors disabled:opacity-40 ${enabled ? 'bg-[#3370FF]' : 'bg-gray-200'}`}
              >
          ...
          {/* ── Config form (shown only when enabled) ── */}
          {enabled && (
            <>
              {/* Presets */}
              {presets.length > 0 && (
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className={`h-11 shrink-0 rounded-xl px-4 text-[13px] font-medium transition-colors disabled:opacity-40 ${
                          validated ? 'bg-green-100 text-green-700' : 'bg-[#EBF0FF] text-[#3370FF]'
                        }`}
                      >
                        {busy
